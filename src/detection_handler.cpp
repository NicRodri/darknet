...
class DetectionHandler{
    ...
    // Extracts the object information from ball, net, and table
    ...
    bool detectBounce(){
        ...
        // Used to start the timer for checking valid bounces
    ...
    // Resets all flags regarding scoring logic
    void resetScoringLogicFlags(){
        lastBounceP1 = false;
        lastBounceP2 = false;
    ...
    void scoringLogic(){
        ...
        if (isBounce){
            ...
            if (std::chrono::duration_cast<std::chrono::seconds>(now - lastBounceTime).count() > 3){
                ...
                resetScoringLogicFlags(); // Reset for the next rally
            }
        }
    }

    // Determines the vertical direction of the ball's movement
    std::string getBallVerticalDirection(){
    ...
    // Used to get the ball movement direction
    std::string ballMovementDetails(){
        std::string direction = getBallVerticalDirection();
        return direction + " (Previous Position: x=" + std::to_string(prevBallPosition.x) +
            ", y=" + std::to_string(prevBallPosition.y) + ")";
    }

    // Displays everything into terminal
    void displayGameInfo(cv::Mat *show_img){
        // Get the movement direction using the modular function
        std::string ballMovementInfo = ballMovementDetails();

        // Display ball, net, and table details
        std::cout << "Ball: x=" << ball.x << ", y=" << ball.y
            << ", w=" << ball.width << ", h=" << ball.height
            << ", Direction: " << ballMovementInfo << std::endl;
        ...
        // Display player scores and bounces
